...
class timeclock(commands.Cog):
    ...
    async def clock_in(self, ctx):
        ...
        if not self.db.check_active(discord_id, guild_id):
            ...
            embed.add_field(
                name="You are not registered with this discord server.",
                value=str(
                    "Please use the 'register' command to sign up before clocking in"
                ),
            )
        ...
        userdata = list(
            users.find({"discord_id": discord_id, "timezone": {"$exists": False}})
        )
        if len(userdata) > 0:
            embed = Embed(
                title="Please use the 'edit' command to update your timezone before clocking in.",
                color=0x000FF,
            )
            await dm.send(embed=embed)
            return
        ...
        if self.db.clock_user_in(discord_id, guild_id):
            ...
            embed = Embed(
                title="There has been an error in your clock in.", color=0x000FF
            )
            await dm.send(embed=embed)
    ...
    async def clock_out(self, ctx):
        ...
        users = self.db.get_employee_records(guild_id)
        userdata = list(
            users.find({"timezone": {"$exists": False}, "discord_id": discord_id})
        )

        if len(userdata) > 0:
            embed = Embed(
                title="Please use the 'edit' command to update your timezone before clocking out.",
                color=0x000FF,
            )
            await dm.send(embed=embed)
            return
        ...
        if self.db.clock_user_out(discord_id, guild_id):
            ...
            embed.add_field(
                name="Out time:",
                value=str(out_time.astimezone(timezone_pytz).strftime("%I:%M:%S %p")),
            )
            ...
            embed = Embed(
                title="There has been an error in your clock out.", color=0x000FF
            )
            await dm.send(embed=embed)
    ...
    async def fix(self, ctx):
        ...
        if len(userdata) > 0:
            embed = Embed(
                title="Please use the 'edit' command to update your timezone first.",
                color=0x000FF,
            )
            await dm.send(embed=embed)
            return
        ...
        if not self.db.check_complete(
            discord_id, guild_id
        ) and not self.db.check_manager(discord_id, guild_id):
            embed = Embed(
                title="You do not have any completed shifts to edit.", color=0x000FF
            )
            await dm.send(embed=embed)
            return
        ...
        for shift in shifts:
            ...
            shift_data.append(
                SelectOption(
                    label=instr + " to " + outstr,
                    value=str(shift["in_time"].timestamp()),
                )
            )
        ...
        await dm.send(
            ...
            embed=embed,
            components=[
                Select(placeholder="Shifts", options=recent_14, custom_id="shift")
            ],
        )
        # await response
                ...
                Select(
                    ...
                    options=[
                        SelectOption(
                            label="In: "
                            + str(
                                in_time.astimezone(tzp).strftime("%I:%M %p on %A %B %d")
                            ),
                            value="in",
                        ),
                        SelectOption(
                            label="Out: "
                            + str(
                                out_time.astimezone(tzp).strftime(
                                    "%I:%M %p on %A %B %d"
                                )
                            ),
                            value="out",
        ...
        while try_again:
            ...
            if choice1.values[0] == "in":
                for x in range(24):
                    ...
                    if not out_time.astimezone(tzp).strftime("%d") == val.astimezone(
                        tzp
                    ).strftime("%d"):
                ...
                for x in range(24):
                    ...
                    if not in_time.astimezone(tzp).strftime("%d") == val.astimezone(
                        tzp
                    ).strftime("%d"):
            ...
            await dm.send(
                ...
                embed=embed,
                components=[
                    Select(placeholder="Hours", options=ops, custom_id="hours")
                ],
            )
            # await response
            ...
            if choice1.values[0] == "in":
                hourstr = (
                    out_time.astimezone(tzp) - timedelta(hours=hoursint)
                ).strftime("%I")
            else:
                hourstr = (
                    in_time.astimezone(tzp) + timedelta(hours=hoursint)
                ).strftime("%I")
            ...
            embed = Embed(
                title="Select a minute value for this time entry.", color=0x000FF
            )
            await dm.send(
                "",
                embed=embed,
            ...
            if choice.component.custom_id == "exit":
                ...
                if self.db.check_timestamp_collision_change(
                    new_dt, in_time, out_time, discord_id, guild_id
                ):
                    embed = Embed(
                ...
                if choice1.values[0] == "in":
                    ...
                    if self.db.check_manager(str(ctx.author), guild_id):
                        ...
                        emp_name = self.db.get_employee_records(guild_id).find_one(
                            {"discord_id": discord_id}
                        )
                        boss_name = self.db.get_employee_records(guild_id).find_one(
                            {"discord_id": str(ctx.author)}
                        )
                        emp_str = emp_name["name_first"] + " " + emp_name["name_last"]
                        boss_str = (
                            boss_name["name_first"] + " " + boss_name["name_last"]
                        )
                        new_val = {
                        ...
                        embed = Embed(
                            ...
                            + str(emp_str)
                            + " for a shift on "
                            + in_time.astimezone(tzp).strftime("%A %B %d %Y"),
                            color=0x000FF,
                        )
                        embed.add_field(
                            name="Old Shift:",
                            value=dt_change.astimezone(tzp).strftime("%I:%M:%S %p")
                            + " to "
                            + str(out_time.astimezone(tzp).strftime("%I:%M:%S %p")),
                        )
                        embed.add_field(
                            name="New shift:",
                            value=new_dt.astimezone(tzp).strftime("%I:%M:%S %p")
                            + " to "
                            + str(out_time.astimezone(tzp).strftime("%I:%M:%S %p")),
                        )
                        await self.post_mng_log_embed(ctx, embed)
                        ...
                        embed = Embed(
                            title="Please wait while we aprove this with your boss.",
                            color=0x000FF,
                        )
                        # await response
                        await dm.send(embed=embed)
                        ...
                        if boss_choice.component.custom_id == "yes":
                            ...
                            await dm.send(
                                embed=Embed(
                                    title=str(boss_str)
                                    + " has declined your timeclock change. Please contact them \
                        ...
                        embed = Embed(
                            title=boss_str
                            + " has confirmed your timeclock change on "
                    ...
                    if self.db.check_manager(str(ctx.author), guild_id):
                        ...
                        embed.add_field(
                            name="Old Shift:",
                            value=in_time.astimezone(tzp).strftime("%I:%M:%S %p")
                            + " to "
                        ...
                        await dm.send(
                            embed=Embed(
                                title="Please wait while we aprove this with a manager.",
                                color=0x000FF,
                            )
                        )
                        boss_choice = await self.client.wait_for(
    ...
    async def data(self, ctx):
        ...
        if self.db.check_manager(user_id, guild_id):
            ...
            embed = Embed(
                title="Whos timeclock entries would you like to view?", color=0x000FF
            )
            await dm.send(
                "",
                embed=embed,
        ...
        await dm.send(
            embed=Embed(title="Select a starting month", color=0x000FF),
        ...
        await dm.send(
            embed=Embed(title="Select a starting day", color=0x000FF),
        ...
        await dm.send(
            embed=Embed(title="Select an ending month", color=0x000FF),
            components=[Select(placeholder="Month", options=ops, custom_id="Month")],
        )
        ...
        if start.strftime("%m") == end_month:
            ops.append(SelectOption(label="More", value="More"))
        await dm.send(
            embed=Embed(title="Select an ending day", color=0x000FF),
            components=[Select(placeholder="Day", options=ops, custom_id="Day")],
        )
        ...
        shifts = []
        await dm.send(
            embed=Embed(title="Creating and Sending timeclock file.", color=0x000FF)
        )
        for entry in slice:
            if all:
    ...
    async def ten_hour_check(self, ctx):
        ...
        while True:
            ...
            dm = await ctx.author.create_dm()
            await dm.send(
                "You have been clocked in for a long time. Is this intentional?",
                components=[
            ...
            try:
                ans = await self.client.wait_for(
                    "button_click",
                    timeout=300,
                    check=lambda i: i.custom_id == "out" or "remain",
                )
                await self.clear_last_msg(dm)
