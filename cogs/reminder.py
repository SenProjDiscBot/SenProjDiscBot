...
class reminder(commands.Cog):
    ...
    async def reminder(self, ctx):
        ...
        embed = discord.Embed(
            title="When would you like your reminder(in minutes?) ", color=0x000FF
        )
        ...
        while True:
            try:
                ...
                while int(rt.content) < 0:
                    await dm.send(
                        "Please enter a POSITIVE INTEGER for your reminder time"
                    )
                    rt = await self.client.wait_for("message", check=check)
                break
            except:
                embed = discord.Embed(
                    title="Please enter a POSITIVE INTEGER for your reminder time ",
                    color=0x000FF,
                )
                await dm.send(embed=embed)
                rt = await self.client.wait_for("message", check=check)
        ...
        embed = discord.Embed(
            title="What would you like to be reminded of ? ", color=0x000FF
        )
        await dm.send(embed=embed)
        rc = await self.client.wait_for("message", check=check)
        reminder_content = rc.content
        embed = discord.Embed(
            title="you will be reminded to "
            + reminder_content
            + " in "
            + reminder_time
            + " minutes! ",
