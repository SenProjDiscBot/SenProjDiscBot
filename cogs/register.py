...
class register(commands.Cog):
    ...
    async def register(self, ctx):
            ...
            components=[
                Select(
                    ...
                    options=[
                        ...
                        SelectOption(
                            label="AST Alaska Standard Time GMT-9:00", value="US/Alaska"
                        ),
                        SelectOption(
                        ...
                        SelectOption(
                            label="UTC	Universal Coordinated Time GMT+0:00", value="UTC"
                        ),
                        SelectOption(
                        ...
                        SelectOption(
                            label="NET	Near East Time GMT+4:00", value="Asia/Dubai"
                        ),
                        SelectOption(
                            label="PLT	Pakistan Lahore Time GMT+5:00", value="Asia/Oral"
                        ),
                        SelectOption(
                        ...
                        SelectOption(
                            label="JST	Japan Standard Time	GMT+9:00", value="Asia/Tokyo"
                        ),
                        SelectOption(
        ...
        timezone_interaction = await self.client.wait_for(
            "select_option",
            check=lambda i: i.custom_id == "timezone" and i.user == ctx.author,
        )
        timezone = timezone_interaction.values[0]
        await self.clear_last_msg(dm)
        member_id = ctx.author.id
        self.db.add_user(
            discord_id, first_name, last_name, timezone, guild_id, member_id
        )
    ...
    async def promote(self, ctx):
        ...
        guild_id = str(ctx.guild.id)
        dm = await ctx.author.create_dm()
        if discord_id == ctx.guild.owner.id or self.db.check_manager(
            discord_id, guild_id
        ):
            ...
            promote_interaction = await self.client.wait_for(
                "select_option",
                check=lambda i: i.custom_id == "promote" and i.user == ctx.author,
            )
            ...
            await dm.send(
                "You do not have permission to access this command. (peasant)"
            )
    ...
    async def demote(self, ctx):
        ...
        if discord_id == ctx.guild.owner.id or self.db.check_manager(
            ...
            demote_interaction = await self.client.wait_for(
                "select_option",
                check=lambda i: i.custom_id == "demote" and i.user == ctx.author,
            )
