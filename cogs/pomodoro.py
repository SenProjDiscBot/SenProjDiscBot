...
class pomodoro(commands.Cog):
    ...
    async def pomodoro(self, ctx):
        ...
        while i < 16:
            ...
            qrembed2 = discord.Embed(
                description="please take a 5 minute break", color=0x000FF
            )
            await ctx.send(embed=qrembed2)
            await asyncio.sleep(converted_5)
            qrembed3 = discord.Embed(
                description="Your 5 minute pomodoro break is over, please choose another task to focus on for 25 minutes",
                color=0x000FF,
            )
    ...
    async def quitpomodoro(self, ctx):
        ...
        qrembed1 = discord.Embed(
            description="Would you like to end your pomodoro session?", color=0x000FF
        )
