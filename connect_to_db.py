...
class connect_to_db:
    ...
    def add_user(
        self, discord_id, first_name, last_name, timezone, guild_id, member_id
    ):
    ...
    def check_timestamp_collision_change(
        self, utc_timestamp, in_time, out_time, discord_id, guild_id
    ):
        ...
        for record in user_records:
            print(
                record["in_time"].strftime("%m:%d:%Y-%H:%M:%S:%f")
                + " "
                + in_time.strftime("%m:%d:%Y-%H:%M:%S:%f")
            )
            print(
                record["out_time"].strftime("%m:%d:%Y-%H:%M:%S:%f")
                + " "
                + out_time.strftime("%m:%d:%Y-%H:%M:%S:%f")
            )
            if record["in_time"].strftime("%m:%d:%Y-%H:%M:%S:%f") == in_time.strftime(
                "%m:%d:%Y-%H:%M:%S:%f"
            ) and record["out_time"].strftime(
                "%m:%d:%Y-%H:%M:%S:%f"
            ) == out_time.strftime(
                "%m:%d:%Y-%H:%M:%S:%f"
            ):
                continue
            if (
                utc_timestamp >= record["in_time"]
                and utc_timestamp <= record["out_time"]
            ):
                return True
    ...
    def update_all_manager_values(self, guild_id):
        ...
        for employee in tars:
            records.update_one(
                {"discord_id": employee["discord_id"]}, {"$set": {"manager": False}}
            )
