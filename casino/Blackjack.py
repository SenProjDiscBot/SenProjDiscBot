...
class Blackjack():
    def __init__(self):
        ...
        self.names = [self.seat_one_player, self.seat_two_player, self.seat_three_player, self.seat_four_player]
        self.shoe = Shoe(7)
    ...
    async def add_player(self, player_name, ctx):
        if not player_name in self.names:
            ...
            if "Empty" in self.names:
                ...
                ret = "All seats are taken. Sorry " + userdata["name_first"] + ". You will have to wait for a seat to open up."
                await ctx.send(ret)
    ...
    def leave_player(self, player_name, ctx):
        if player_name in self.names:
            ...
            if self.seat_one_player == player_name:
                ...
                self.seat_one_nic = "Empty"
                self.seat_one_taken = False
                records.update_one({'discord_id' : player_name} , {"$set":{'chips' : self.seat_one_pool}})
                self.seat_one_pool = 0
                self.names = [self.seat_one_player, self.seat_two_player, self.seat_three_player, self.seat_four_player]
                ...
                self.seat_two_nic = "Empty"
                self.seat_two_taken = False
                records.update_one({'discord_id' : player_name} , {"$set":{'chips' : self.seat_two_pool}})
                self.seat_two_pool = 0
                self.names = [self.seat_one_player, self.seat_two_player, self.seat_three_player, self.seat_four_player]
                ...
                self.seat_three_nic = "Empty"
                self.seat_three_taken = False
                records.update_one({'discord_id' : player_name} , {"$set":{'chips' : self.seat_three_pool}})
                self.seat_three_pool = 0
                self.names = [self.seat_one_player, self.seat_two_player, self.seat_three_player, self.seat_four_player]
                ...
                self.seat_four_nic = "Empty"
                self.seat_four_taken = False
                records.update_one({'discord_id' : player_name} , {"$set":{'chips' : self.seat_four_pool}})
                self.seat_four_pool = 0
                self.names = [self.seat_one_player, self.seat_two_player, self.seat_three_player, self.seat_four_player]
    ...
    def end_game(self):
        ...
        # case dealer 21 everyone loses
            ...
            embed.add_field(name="Dealer hit blackjack!", value="Players who also hit blackjack will push.")
        if self.seat_one_bet > 0:
            # seat 1 bust
            if self.count(self.seat_one) > 21:
                embed.add_field(name=self.seat_one_nic, value="Player 1 has gone bust...")
            # seat 1 push
            elif self.count(self.seat_one) == self.count(self.dealer):
                self.seat_one_pool = self.seat_one_pool + self.seat_one_bet
                embed.add_field(name=self.seat_one_nic, value="Player 1 pushes with the dealer.")
            #seat 1 win
            elif self.count(self.seat_one) > self.count(self.dealer):
                p1val = ""
                if self.count(self.seat_one) == 21 and len(self.seat_one) == 2:
                    ...
                    p1val = "BLACKJACK!\n"
                self.seat_one_pool = self.seat_one_pool + ( 2 * self.seat_one_bet )
                p1val = p1val + "Player 1 wins " + str(2 * self.seat_one_bet) + " chips!"
        ...
        if self.seat_two_bet > 0:
            ...
            if self.count(self.seat_two) > 21:
                embed.add_field(name=self.seat_two_nic, value="Player 2 has gone bust...")
            # seat 2 push
            elif self.count(self.seat_two) == self.count(self.dealer):
                self.seat_two_pool = self.seat_two_pool + self.seat_two_bet
                embed.add_field(name=self.seat_two_nic, value="Player 2 pushes with the dealer.")
            #seat 2 win
            elif self.count(self.seat_two) > self.count(self.dealer):
                p2val = ""
                if self.count(self.seat_two) == 21 and len(self.seat_two) == 2:
                    ...
                    p2val = "BLACKJACK!\n"
                self.seat_two_pool = self.seat_two_pool + ( 2 * self.seat_two_bet )
                p2val = p2val + "Player 2 wins " + str(2 * self.seat_two_bet) + " chips!"
        ...
        if self.seat_three_bet > 0:
            ...
            if self.count(self.seat_three) > 21:
                embed.add_field(name=self.seat_three_nic, value="Player 3 has gone bust...")
            # seat 3 push
            elif self.count(self.seat_three) == self.count(self.dealer):
                self.seat_three_pool = self.seat_three_pool + self.seat_three_bet
                embed.add_field(name=self.seat_three_nic, value="Player 3 pushes with the dealer.")
            #seat 3 win
            elif self.count(self.seat_three) > self.count(self.dealer):
                p3val = ""
                if self.count(self.seat_three) == 21 and len(self.seat_three) == 2:
                    ...
                    p3val = "BLACKJACK!\n"
                self.seat_three_pool = self.seat_three_pool + ( 2 * self.seat_three_bet )
                p3val = p3val + "Player 3 wins " + str(2 * self.seat_three_bet) + " chips!"
        ...
        if self.seat_four_bet > 0:
            ...
            if self.count(self.seat_four) > 21:
                embed.add_field(name=self.seat_four_nic, value="Player 4 has gone bust...")
            # seat 4 push
            elif self.count(self.seat_four) == self.count(self.dealer):
                self.seat_four_pool = self.seat_four_pool + self.seat_four_bet
                embed.add_field(name=self.seat_four_nic, value="Player 4 pushes with the dealer.")
            #seat 4 win
            elif self.count(self.seat_three) > self.count(self.dealer):
                p4val = ""
                if self.count(self.seat_four) == 21 and len(self.seat_four) == 2:
                    ...
                    p4val = "BLACKJACK!\n"
                self.seat_four_pool = self.seat_four_pool + ( 2 * self.seat_four_bet )
                p4val = p4val + "Player 4 wins " + str(2 * self.seat_four_bet) + " chips!"
    ...
    def player_hit(self, player_name):
        if player_name in self.names:
            if self.seat_one_player == player_name and self.seat_one_bet > 0 and self.hand_counter(player_name) < 21:
                self.seat_one.append(self.shoe.deal_card())
                if self.count(self.seat_one) >= 21:
                    self.seat_one_out = True

            elif self.seat_two_player == player_name and self.seat_two_bet > 0 and self.hand_counter(player_name) < 21:
                self.seat_two.append(self.shoe.deal_card())
                if self.count(self.seat_two) >= 21:
                    self.seat_two_out = True

            elif self.seat_three_player == player_name and self.seat_three_bet > 0 and self.hand_counter(player_name) < 21:
                self.seat_three.append(self.shoe.deal_card())
                if self.count(self.seat_three) >= 21:
                    self.seat_three_out = True

            elif self.seat_four_player == player_name and self.seat_four_bet > 0 and self.hand_counter(player_name) < 21:
                self.seat_four.append(self.shoe.deal_card())
                if self.count(self.seat_four) >= 21:
                    self.seat_four_out = True
    ...
    def check_chips_created(self, ctx):
        ...
        for user in userdata:
            users.update_one({'discord_id' : user['discord_id']} , {"$set":{'chips' : 1000}})